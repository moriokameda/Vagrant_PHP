...
class Poll {
    ...
    private function _save() {
        $sql = "insert into answers
                (answer, created, remote_address, user_agent, answer_date)
                values (:answer, now(), :remote_address, :user_agent, now())";
        $stmt = $this->_db->prepare($sql);
        $stmt->bindValue(':answer', (int)$_POST['answer'], \PDO::PARAM_INT);
        $stmt->bindValue(':remote_address', $_SERVER['REMOTE_ADDRESS'], \PDO::PARAM_STR);
